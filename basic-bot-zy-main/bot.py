...
class RolesReaction(commands.InteractionBot):
    def __init__(self):
        ...
        self.db = database.Database(f"{self.directory}/files/data.db")
        self.version = version.get(self.directory)
        self.response = Response(self, f"{self.directory}/i18n", self.config.language)
        intents = disnake.Intents(message_content=True, guild_messages=True, guild_reactions=True, guilds=True, members=True)
    ...
    async def database_updates(self):
        ...
        handler = schema.SchemaHandler(f"{self.directory}/files/data.db", self)
